...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> loadDashboard() async { try { final r=await ApiService.get('dashboard'); final d=r is Map&&r['data'] is Map?r['data']:r; if(!mounted)return; setState((){data=Map<String,dynamic>.from(d);loading=false;error=null;}); } catch(e){if(mounted)setState((){loading=false;error=e.toString().replaceFirst('Exception: ','');});} }
  ...
  Widget stat(String title,dynamic value,IconData icon,Widget page)=>Card(child:InkWell(borderRadius:BorderRadius.circular(12),onTap:()=>open(page),child:Padding(padding:const EdgeInsets.all(12),child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[Icon(icon,color:AppColors.red,size:28),Text('${value??0}',style:const TextStyle(fontSize:22,fontWeight:FontWeight.bold)),Text(title),const SizedBox(height:4),const Icon(Icons.touch_app,size:16)]))));
  @override Widget build(BuildContext context){
    final menu=<Widget>[menuItem(Icons.people,'الطلاب',const StudentsScreen()),menuItem(Icons.family_restroom,'أولياء الأمور',const ParentsScreen()),menuItem(Icons.school,'المدرسون',const TeachersScreen()),menuItem(Icons.supervisor_account,'المشرفون',const SupervisorsScreen()),menuItem(Icons.groups,'المجموعات',const GroupsScreen()),menuItem(Icons.menu_book,'الدروس والحصص',const LessonsScreen()),menuItem(Icons.fact_check,'المتابعة الأكاديمية',const AcademicTrackingScreen()),menuItem(Icons.event_available,'الاشتراكات',const SubscriptionsScreen()),menuItem(Icons.payments,'المدفوعات',const PaymentsScreen()),const Divider(),menuItem(Icons.calendar_month,'الجداول',const SchedulesScreen()),menuItem(Icons.fact_check_outlined,'الحضور',const AttendanceScreen()),menuItem(Icons.money_off,'المصروفات',const ExpensesScreen()),menuItem(Icons.account_balance_wallet,'المالية',const FinanceScreen()),menuItem(Icons.bar_chart,'التقارير',const ReportsScreen()),menuItem(Icons.admin_panel_settings,'المستخدمون والصلاحيات',const UsersScreen())];
    Widget body;
