...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> loadDashboard() async { try { final r=await ApiService.get('dashboard'); if(!mounted)return; final d=r is Map&&r['data'] is Map?r['data']:r; setState((){data=Map<String,dynamic>.from(d);loading=false;error=null;}); } catch(e){if(mounted)setState((){loading=false;error=e.toString().replaceFirst('Exception: ','');});} }
  ...
  @override Widget build(BuildContext context){
    ...
    if(loading){body=const Center(child:CircularProgressIndicator());}
      ...
      final cards=<Widget>[
        ...
        step(1,'الطلاب والمواد','أضف الطالب واختر المواد المشترك بها.',Icons.people,const StudentsScreen()),
        step(2,'أولياء الأمور','اجمع أبناء ولي الأمر الواحد في ملف واحد واعرض تفاصيلهم.',Icons.family_restroom,const ParentsScreen()),
        step(3,'المدرسون والربط','اربط المدرس بالطالب والمادة وحدد سعر الحصة.',Icons.school,const TeachersScreen()),
        step(4,'المجموعات والدروس','أنشئ المجموعة أو الحصة بعد اكتمال العلاقات.',Icons.groups,const GroupsScreen()),
        const SizedBox(height:10),const Text('ملخص الأكاديمية',style:TextStyle(fontWeight:FontWeight.bold,fontSize:18)),const SizedBox(height:6),
        GridView.count(shrinkWrap:true,physics:const NeverScrollableScrollPhysics(),crossAxisCount:2,crossAxisSpacing:10,mainAxisSpacing:10,children:[
          stat('الطلاب',data?['students'],Icons.people,const StudentsScreen()),stat('المدرسون',data?['teachers'],Icons.school,const TeachersScreen()),stat('حصص اليوم',data?['today_classes'],Icons.calendar_today,const LessonsScreen()),stat('حضور اليوم',data?['today_attendance'],Icons.fact_check,const AttendanceScreen()),stat('دخل الشهر',data?['monthly_income'],Icons.payments,const PaymentsScreen()),stat('مصروفات الشهر',data?['monthly_expenses'],Icons.money_off,const ExpensesScreen()),stat('تجديد خلال 7 أيام',data?['expiring_7_days'],Icons.warning,const SubscriptionsScreen())
        ])
      ];
      body=RefreshIndicator(onRefresh:loadDashboard,child:ListView(padding:const EdgeInsets.fromLTRB(16,12,16,30),children:cards));
    }
