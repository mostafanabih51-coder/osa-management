...
class _SchedulesScreenState extends State<SchedulesScreen> {
  ...
  Future<void> load() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        schedules = List<dynamic>.from(a[0]['data'] ?? a[0]);
        teachers = List<dynamic>.from(a[1]['data'] ?? a[1]);
        students = List<dynamic>.from(a[2]['data'] ?? a[2]);
        loading = false;
        error = null;
      });
    } catch (e) {
  ...
  Future<void> add() async {
    ...
    int? studentId = students.isEmpty ? null : students.first['id'];
    final group = TextEditingController();
    final subject = TextEditingController();
    final start = TextEditingController();
    final end = TextEditingController();
    final zoom = TextEditingController();
              ...
              child: Column(
                ...
                children: [
                  DropdownButtonFormField<String>(
                    value: type,
                    items: const [
                      DropdownMenuItem(value: 'private', child: Text('فردية')),
                      DropdownMenuItem(value: 'group', child: Text('مجموعة')),
                    ],
                    onChanged: (v) { if (v != null) setD(() => type = v); },
                  ...
                  if (type == 'private' && students.isNotEmpty)
                    DropdownButtonFormField<int>(
                      value: studentId,
                      items: students.map((x) => DropdownMenuItem<int>(
                        value: x['id'], child: Text('${x['name'] ?? ''}'),
                      )).toList(),
                      onChanged: (v) => setD(() => studentId = v),
                  ...
                  if (type == 'group')
                    TextFormField(
                      ...
                      validator: (v) => v!.trim().isEmpty ? 'اكتب اسم المجموعة' : null,
                    ),
                  TextFormField(
                    controller: subject,
                    decoration: const InputDecoration(labelText: 'المادة'),
                    validator: (v) => v!.trim().isEmpty ? 'اكتب المادة' : null,
                  ),
                  TextFormField(
                    controller: start,
                  ...
                  TextFormField(
                    controller: zoom,
                    decoration: const InputDecoration(labelText: 'رابط Zoom (اختياري)'),
                  ),
                ],
              ),
            ),
    ...
    if (ok != true) return;
    try {
      await ApiService.post('schedules', {
        'type': type,
        'teacher_id': teacherId,
        'student_id': type == 'private' ? studentId : null,
        'group_name': type == 'group' ? group.text.trim() : null,
        'subject': subject.text.trim(),
        'starts_at': start.text.trim(),
        'ends_at': end.text.trim(),
        'zoom_url': zoom.text.trim().isEmpty ? null : zoom.text.trim(),
      });
      await load();
    } catch (e) {
              ...
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final t = x['teacher'];
                      final s = x['student'];
                      final teacherName = t is Map ? '${t['name'] ?? ''}' : 'غير محدد';
                      final studentName = s is Map ? '${s['name'] ?? ''}' : '';
                      final groupName = x['group_name'];
                      final lines = [
                        '${x['starts_at'] ?? ''}',
                        'المدرس: $teacherName',
                        if (studentName.isNotEmpty) 'الطالب: $studentName',
                        if (groupName != null) 'المجموعة: $groupName',
                      ].join('\n');
                      return Card(
                        child: ListTile(
                          leading: const CircleAvatar(child: Icon(Icons.calendar_month)),
                          title: Text('${x['subject'] ?? 'حصة'}'),
                          subtitle: Text(lines),
                        ),
                      );
                    },
