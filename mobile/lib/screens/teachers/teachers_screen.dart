...
class _TeachersScreenState extends State<TeachersScreen> {
  ...
  List<String> studentSubjects(dynamic student) => list(student is Map ? student['subjects'] : null)
  ...
  Future<void> load() async {
    ...
    try {
      subjects = list(await ApiService.get('subjects')).map((x) => x is Map ? '${x['subject'] ?? x['name'] ?? ''}' : '$x').where((x) => x.trim().isNotEmpty).toSet().toList();
    ...
    if (!mounted) return;
    setState(() { loading = false; loadError = teachers.isEmpty && firstError != null ? '$firstError'.replaceFirst('Exception: ', '') : null; });
  }
  ...
  Future<void> addOrEdit({Map<String,dynamic>? existing}) async {
    ...
    if (ok == true) {
      try {
        final body = {'name': name.text.trim(), 'phone': phone.text.trim(), 'email': email.text.trim(), 'specialization': specialization.text.trim(), 'status': 'active'};
        if (existing == null) await ApiService.post('teachers', body); else await ApiService.put('teachers/${id(existing)}', body);
        await load();
      } catch (e) { msg(e); }
    }
  ...
  Future<void> remove(dynamic teacher) async {
    final ok = await showDialog<bool>(context: context, builder: (d) => AlertDialog(
      title: const Text('حذف المدرس'), content: Text('حذف ${teacher['name'] ?? ''}؟'),
      actions: [TextButton(onPressed: () => Navigator.pop(d, false), child: const Text('إلغاء')), FilledButton(onPressed: () => Navigator.pop(d, true), child: const Text('حذف'))],
    ));
    if (ok == true) { try { await ApiService.delete('teachers/${id(teacher)}'); await load(); } catch (e) { msg(e); } }
  }

  Future<void> assign(dynamic teacher) async {
    if (students.isEmpty) { msg('لا يوجد طلاب محملون. افتح الطلاب وتأكد من اتصال التطبيق ثم أعد المحاولة.'); return; }
    final available = students.where((s) => studentSubjects(s).isNotEmpty).toList();
    if (available.isEmpty) { msg('أضف مادة واحدة على الأقل للطالب قبل ربطه بالمدرس.'); return; }
    ...
    final ok = await showDialog<bool>(context: context, builder: (d) => StatefulBuilder(builder: (c, set) {
      final filtered = candidates();
      if (!filtered.any((s) => id(s) == studentId)) studentId = filtered.isNotEmpty ? id(filtered.first) : 0;
      return AlertDialog(
        title: Text('ربط طالب بمدرس: ${teacher['name'] ?? ''}'),
        content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
          DropdownButtonFormField<String>(value: subject, items: subjects.where((s) => available.any((st) => studentSubjects(st).contains(s))).map((s) => DropdownMenuItem(value: s, child: Text(s))).toList(), onChanged: (v) { if (v != null) set(() { subject = v; final f = candidates(); studentId = f.isEmpty ? 0 : id(f.first); }); }, decoration: const InputDecoration(labelText: 'المادة المشترك بها الطالب')),
          const SizedBox(height: 8),
          DropdownButtonFormField<int>(value: filtered.isEmpty ? null : studentId, items: filtered.map((s) => DropdownMenuItem(value: id(s), child: Text('${s['name'] ?? ''}'))).toList(), onChanged: (v) { if (v != null) set(() => studentId = v); }, decoration: const InputDecoration(labelText: 'الطالب')),
          ...
          DropdownButtonFormField<int?>(value: supervisorId, items: <DropdownMenuItem<int?>>[const DropdownMenuItem(value: null, child: Text('بدون مشرف')), ...supervisors.map((s) => DropdownMenuItem(value: id(s), child: Text('${s['name'] ?? ''}')))], onChanged: (v) => set(() => supervisorId = v), decoration: const InputDecoration(labelText: 'المشرف (اختياري)')),
        ])),
        actions: [TextButton(onPressed: () => Navigator.pop(d, false), child: const Text('إلغاء')), FilledButton(onPressed: () { if (studentId > 0 && double.tryParse(rate.text) != null) Navigator.pop(d, true); else msg('اختر طالبًا وأدخل سعر المدرس.'); }, child: const Text('حفظ'))],
      );
    }));
    if (ok == true) {
      try {
        await ApiService.post('teacher-assignments', {'teacher_id': id(teacher), 'student_id': studentId, 'subject': subject, 'teacher_rate': double.parse(rate.text), 'academy_percentage': double.tryParse(percentage.text) ?? 0, 'supervisor_id': supervisorId, 'status': 'active'});
        await load();
        msg('تم ربط الطالب بالمدرس والمادة والسعر.');
      } catch (e) { msg(e); }
    }
    rate.dispose(); percentage.dispose();
  }
  ...
  Future<void> details(int teacherId) async {
    try {
      ...
      final resources = list(await ApiService.get('academic/resources?teacher_id=$teacherId'));
      if (!mounted) return;
      await showDialog<void>(context: context, builder: (d) => AlertDialog(
        ...
        content: SizedBox(width: 620, child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          const Divider(), Text('الطلاب المرتبطون (${linked.length})', style: const TextStyle(fontWeight: FontWeight.bold)),
          ...linked.map((s) => ListTile(dense: true, contentPadding: EdgeInsets.zero, title: Text('${s['name'] ?? ''}'), subtitle: Text(s['pivot'] is Map ? '${s['pivot']['subject'] ?? ''} • سعر ${s['pivot']['teacher_rate'] ?? 0}' : '')),
          const Divider(), Text('الحصص (${lessons.length})', style: const TextStyle(fontWeight: FontWeight.bold)),
          ...lessons.take(20).map((l) => Text('• ${l['subject'] ?? ''} — ${l['starts_at'] ?? ''} — ${l['status'] ?? ''}')),
          const Divider(), Text('المرفقات (${resources.length})', style: const TextStyle(fontWeight: FontWeight.bold)),
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(onPressed: () => addOrEdit(), backgroundColor: AppColors.red, icon: const Icon(Icons.person_add), label: const Text('إضافة مدرس')),
      body: loading ? const Center(child: CircularProgressIndicator()) : RefreshIndicator(onRefresh: load, child: loadError != null
          ? ListView(children: [const SizedBox(height: 160), Center(child: Padding(padding: EdgeInsets.all(24), child: Text('تعذر تحميل المدرسين. اضغط تحديث للمحاولة مرة أخرى.', textAlign: TextAlign.center))), Center(child: FilledButton(onPressed: load, child: const Text('إعادة المحاولة')))])
          : ListView(padding: const EdgeInsets.fromLTRB(16,12,16,90), children: [
              ...
              ...teachers.map((teacher) => Card(child: ListTile(onTap: () => details(id(teacher)), leading: const CircleAvatar(child: Icon(Icons.school)), title: Text('${teacher['name'] ?? ''}', style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('المادة: ${teacher['specialization'] ?? '-'}\n${teacher['phone'] ?? teacher['email'] ?? ''}'), isThreeLine: true, trailing: Row(mainAxisSize: MainAxisSize.min, children: [IconButton(onPressed: () => assign(teacher), icon: const Icon(Icons.link)), IconButton(onPressed: () => addOrEdit(existing: Map<String,dynamic>.from(teacher)), icon: const Icon(Icons.edit)), IconButton(onPressed: () => remove(teacher), icon: const Icon(Icons.delete))]))),
            ])),
    );
  }
}
