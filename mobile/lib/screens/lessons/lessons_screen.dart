...
class _LessonsScreenState extends State<LessonsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('الدروس والحصص'), actions: [IconButton(onPressed: loading ? null : load, icon: const Icon(Icons.refresh))]),
      ...
      body: loading ? const Center(child: CircularProgressIndicator()) : error != null ? _error() : lessons.isEmpty ? const Center(child: Text('لا توجد حصص مسجلة')) : RefreshIndicator(
        ...
        child: ListView.builder(padding: const EdgeInsets.all(16), itemCount: lessons.length, itemBuilder: (_, i) {
          ...
          return Card(margin: const EdgeInsets.only(bottom: 10), child: ListTile(
            ...
            title: Text('${x['subject'] ?? ''}', style: const TextStyle(fontWeight: FontWeight.bold)),
            subtitle: Text('${t is Map ? (t['name'] ?? '') : ''} • ${s is Map ? (s['name'] ?? 'حصة مجموعة') : 'حصة'}\n${x['starts_at'] ?? ''} → ${x['ends_at'] ?? ''}'),
            isThreeLine: true,
            trailing: Text('${x['teacher_due'] ?? 0}'),
          ));
        }),
      ),
    );
