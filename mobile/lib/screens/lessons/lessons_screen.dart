...
class _LessonsScreenState extends State<LessonsScreen> {
  ...
  Future<void> load() async { if(mounted)setState((){loading=true;error=null;}); try{final r=await Future.wait([ApiService.get('lessons'),ApiService.get('teachers'),ApiService.get('students'),ApiService.get('supervisors')]);if(!mounted)return;setState((){lessons=r[0]['data']??[];teachers=r[1]['data']??[];students=r[2]['data']??[];supervisors=r[3]['data']??[];loading=false;});}catch(e){if(!mounted)return;setState((){loading=false;error=e.toString().replaceFirst('Exception: ','');});} }
  Future<void> addLesson() async {
    ...
    String type='private';int teacherId=teachers.first['id'];int? studentId=students.isEmpty?null:students.first['id'];int? supervisorId=supervisors.isEmpty?null:supervisors.first['id'];final groupId=TextEditingController();final subject=TextEditingController();final starts=TextEditingController(text:'${DateTime.now().toIso8601String().substring(0,10)} 16:00');final ends=TextEditingController(text:'${DateTime.now().toIso8601String().substring(0,10)} 17:00');final rate=TextEditingController(text:'0');final key=GlobalKey<FormState>();
    final ok=await showDialog<bool>(context:context,builder:(context)=>StatefulBuilder(builder:(context,setD)=>AlertDialog(title:const Text('إضافة حصة'),content:Form(key:key,child:SingleChildScrollView(child:Column(mainAxisSize:MainAxisSize.min,children:[
      ...
      TextFormField(controller:subject,decoration:const InputDecoration(labelText:'المادة'),validator:(v)=>v==null||v.trim().isEmpty?'أدخل المادة':null),TextFormField(controller:starts,decoration:const InputDecoration(labelText:'البداية YYYY-MM-DD HH:MM')),TextFormField(controller:ends,decoration:const InputDecoration(labelText:'النهاية YYYY-MM-DD HH:MM')),TextFormField(controller:rate,keyboardType:TextInputType.number,decoration:const InputDecoration(labelText:'مستحق المدرس')),
