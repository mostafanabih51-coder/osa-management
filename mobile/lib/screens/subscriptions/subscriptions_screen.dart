...
class _SubscriptionsScreenState extends State<SubscriptionsScreen> {
  ...
  Future<void> loadSubscriptions() async {
    ...
    try {
      final results = await Future.wait([ApiService.get('subscriptions'), ApiService.get('students')]);
      if (!mounted) return;
      setState(() { subscriptions = results[0]['data'] ?? []; students = results[1]['data'] ?? []; loading = false; });
  ...
  Future<void> addSubscription() async {
    ...
    final subject = TextEditingController(); final amount = TextEditingController();
    ...
    final ok = await showDialog<bool>(context: context, builder: (context) => AlertDialog(title: const Text('إضافة اشتراك'), content: Form(key: key, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    if (ok != true) return;
    try { await ApiService.post('subscriptions', {'student_id': studentId, 'subject': subject.text.trim(), 'amount': double.parse(amount.text), 'starts_on': starts.text.trim(), 'ends_on': ends.text.trim(), 'status': 'active'}); await loadSubscriptions(); if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم إضافة الاشتراك'))); }
    catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString().replaceFirst('Exception: ', '')))); }
  }

  @override Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('الاشتراكات'), actions: [IconButton(onPressed: loading ? null : loadSubscriptions, icon: const Icon(Icons.refresh))]), floatingActionButton: FloatingActionButton(onPressed: addSubscription, child: const Icon(Icons.add)), body: loading ? const Center(child: CircularProgressIndicator()) : error != null ? _error() : subscriptions.isEmpty ? RefreshIndicator(onRefresh: loadSubscriptions, child: ListView(children: const [SizedBox(height:240), Center(child: Text('لا توجد اشتراكات حاليًا'))])) : RefreshIndicator(onRefresh: loadSubscriptions, child: ListView.builder(padding: const EdgeInsets.all(16), itemCount: subscriptions.length, itemBuilder: (_, i) { final item = subscriptions[i] as Map<String,dynamic>; final student = item['student']; final name = student is Map ? '${student['name'] ?? 'طالب'}' : 'طالب'; return Card(margin: const EdgeInsets.only(bottom:10), child: ListTile(leading: const CircleAvatar(child: Icon(Icons.event_available)), title: Text(name, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('المادة: ${item['subject'] ?? ''}\n${item['starts_on'] ?? ''} → ${item['ends_on'] ?? ''}'), isThreeLine: true, trailing: Text('${item['amount'] ?? 0}')); }));
