...
{
 public function updateTeacher(Request $r,Teacher $teacher){$teacher->update($r->validate(['name'=>'sometimes|required','phone'=>'nullable','email'=>'nullable|email','specialization'=>'nullable','hourly_rate'=>'nullable|numeric|min:0','status'=>'nullable','billing_type'=>'nullable|in:per_lesson,monthly','lesson_price'=>'nullable|numeric|min:0','lesson_count'=>'nullable|integer|min:1|max:100','service_type'=>'nullable|in:private,group']));return response()->json(['success'=>true,'data'=>$teacher->fresh()]);}
 public function destroyTeacher(Teacher $teacher){$teacher->delete();return['success'=>true];}
 public function updateSubscription(Request $r,Subscription $subscription){$d=$r->validate(['student_id'=>'sometimes|exists:students,id','subject'=>'sometimes|required','amount'=>'sometimes|numeric|min:0','starts_on'=>'sometimes|date','ends_on'=>'sometimes|date|after_or_equal:starts_on','status'=>'nullable']);if(isset($d['student_id'])&&$d['student_id']!=$subscription->student_id){$subscription->student_id=$d['student_id'];} $subscription->update($d);return response()->json(['success'=>true,'data'=>$subscription->fresh()->load(['student','payments'])]);}
